...
int qualityconfig = 5;
int buffersconfig = 3;
int avi_length = 120;     // how long a movie in seconds -- 1800 sec = 30 min
...
int start_record = 0;
int start_extract = 0;
int start_extract_2nd_opinion = -2;
int start_extract_1st_opinion = -1;
bool buttonPressed = false;
char extracted_files[4][64];  // Mảng để lưu tên của 2 tệp gần nhất
int extracted_index = 0;      // Biến theo dõi vị trí trong mảng
...
char avi_file_name_extract[100];
static int i = 0;
...
static void config_camera() {
  ...
  config.xclk_freq_hz = 10000000;  // 10000000 or 20000000 -- 100 is faster with v1.04  // 200 is faster with v1.06 // 16500000 is an option
...
//________________Extract 10S before and after press button______________
/*When press button -  cal onButtonPress() function*/
// void onButtonPress(int current_file_number, int file_group, const char* devname) {
//     Serial.println("Access to the onButtonPress function!!");

//     // Xác định các file tương ứng với 10 tệp trước và sau
//     int start_file = current_file_number - 4;
//     int end_file = current_file_number + 4;

//     char avi_file_name[64];

//     // Trích xuất các file trong khoảng từ 10 file trước đến 10 file sau
//     for (int i = start_file; i <= end_file; i++) {
//         Serial.println("Access to the For loop on onButtonPress function!!");

//         // Tạo tên file theo cấu trúc định dạng yêu cầu
//         sprintf(avi_file_name, "/%s%d.%03d.avi", devname, file_group, i);

//         // Kiểm tra và mở file từ thẻ SD
//         File avifile = SD_MMC.open(avi_file_name, "r");
//         if (avifile) {
//             Serial.printf("Processing file: %s\n", avi_file_name);
//             // Xử lý file, chẳng hạn như sao chép hoặc lưu trữ lại
//             avifile.close();
//         } else {
...
//_____________New Update _ Scan Videos__________________
void onButtonPress() {
   File root = SD_MMC.open("/");
  if (!root) {
  ...
  if (!file) {
    Serial.println("Failed to open first file");
    root.close();
    return;
  }
  ...
  while (file) {
    if (!file.isDirectory()) {
      ...
      // Kiểm tra nếu file có đuôi .avi
      if (filename.endsWith(".avi")) {
        // Phân tích tên file để lấy group và number
        int group = -1;
        int number = -1;
        
        // Format file là /{devname}{group}.{number}.avi
        int dotPos = filename.lastIndexOf('.');
        if (dotPos > 0) {
          int secondDotPos = filename.lastIndexOf('.', dotPos - 1);
          if (secondDotPos > 0) {
            // Lấy group number (bỏ qua devname)
            String groupStr = filename.substring(secondDotPos - 3, secondDotPos);
            // Lấy file number
            String numberStr = filename.substring(secondDotPos + 1, dotPos);
            
            group = groupStr.toInt();
            number = numberStr.toInt();
            
            Serial.printf("Found file: %s, group: %d, number: %d\n", 
                         filename.c_str(), group, number);

            // Cập nhật file mới nhất
            if (group > latest_group || 
               (group == latest_group && number > latest_number)) {
              latest_group = group;
              latest_number = number;
              sprintf(latest_file, "/%s", filename.c_str());
              Serial.printf("New latest file: %s (group: %d, number: %d)\n", 
                          latest_file, latest_group, latest_number);
            }
          }
        }
      }
  ...
  // Xử lý file mới nhất tìm được
  if (strlen(latest_file) > 0) {
    Serial.printf("Final latest file: %s\n", latest_file);
    File avifile = SD_MMC.open(latest_file, "r");
    if (avifile) {
      Serial.printf("Successfully opened file: %s, size: %d bytes\n", 
                   latest_file, avifile.size());
      // Xử lý file ở đây
      avifile.close();
    } else {
...
static void start_avi() {
  ...
  // Lưu tên file vào danh sách nếu nút đã nhấn
  //file_number thì cần 1 cái file_number hiện tại và 1 file_number quá khứ
  sprintf(avi_file_name_extract, "/%s%d.%03d.avi", devname, file_group, file_number--);

  if (buttonPressed && extracted_index < 3) {
    strcpy(extracted_files[extracted_index], avi_file_name);
    Serial.print("Stored file name in extracted_files[");
    Serial.print(extracted_index);
    Serial.print("]: ");
    Serial.println(extracted_files[extracted_index]);
    extracted_index++;
  }
...
static esp_err_t restart_handler(httpd_req_t *req) {
  ...
  Serial.print("http restart, core ");
  Serial.print(xPortGetCoreID());
  Serial.print(", priority = ");
  Serial.println(uxTaskPriorityGet(NULL));
  //Serial.printf("Internal Total heap %d, internal Free Heap %d\n", ESP.getHeapSize(), ESP.getFreeHeap());
...
static esp_err_t stream_82_handler(httpd_req_t *req) {
  ...
  Serial.print("stream_82_handler, core ");
  Serial.print(xPortGetCoreID());
  Serial.print(", priority = ");
  Serial.println(uxTaskPriorityGet(NULL));
...
static esp_err_t stream_81_handler(httpd_req_t *req) {
  ...
  Serial.print("stream_81_handler, core ");
  Serial.print(xPortGetCoreID());
  Serial.print(", priority = ");
  Serial.println(uxTaskPriorityGet(NULL));
...
void start_Stream_81_server() {
  ...
  Serial.print("http Stream task prio: ");
  Serial.println(config2.task_priority);
...
void the_camera_loop(void *pvParameter) {
  ...
  frame_cnt = 0;
  start_extract_2nd_opinion = digitalRead(12);
  start_extract_1st_opinion = digitalRead(12);
  start_record = 1;
  ...
  while (1) {
    ...
    if (frame_cnt == 0 && start_record == 0) {
    ...
    } else if (start_record == 1 && buttonPressed == 1) {
      Serial.println("**** Access to the release button press ****");
      // if (buttonPressed) {
      //   strcpy(extracted_files[extracted_index], avi_file_name);
      //   Serial.print("Stored file name in extracted_files[");
      //   Serial.print(extracted_index);
      ...
      //   strcpy(extracted_files[extracted_index], avi_file_name_extract);
      //   Serial.print("Stored file name in extracted_files[");
      //   Serial.print(extracted_index++);
      //   Serial.print("]: ");
      //   Serial.println(extracted_files[extracted_index]);
...
void loop() {
  ...
  // Xóa file cũ nếu cờ được đặt
  if (delete_old_stuff_flag == 1) {
    delete_old_stuff_flag = 0;
    delete_old_stuff();
    Serial.println("___________________________");
    Serial.println("Run processing delete file");
    Serial.println("___________________________");
  }

  // Biến để theo dõi trạng thái nút bấm
  static int last_button_state = HIGH;         // Trạng thái nút trước đó
  int current_button_state = digitalRead(12);  // Đọc trạng thái hiện tại của nút bấm

  // Kiểm tra nếu trạng thái nút thay đổi từ chưa nhấn sang nhấn
  if (current_button_state == LOW && last_button_state == LOW) {
    // Nút bấm đã được nhấn
    buttonPressed = true;  // Bắt đầu quá trình ghi hình hoặc xử lý
  } else {
    buttonPressed = false;  // Không ghi hình
  }
